...
ServiceV2::ServiceV2(string service_type, string service_id, int service_port, int testbed_port)
...
void ServiceV2::Run(void (*error_check)())
{
	if (m_ShouldShutDown)
...

	{
		...
		std::thread safety(&ServiceV2::MonitorSafety, this);
		std::thread heartbeat(&ServiceV2::MonitorHeartbeats, this);
...
void ServiceV2::CleanupAttributes()
{
	m_Properties.clear();
	m_Commands.clear();
	m_DataStreams.clear();
...
void ServiceV2::Close()
{
}

void ServiceV2::ShutDown()
{
	m_ShouldShutDown = true;
}

bool ServiceV2::ShouldShutDown()
{
	return m_ShouldShutDown;
}

bool ServiceV2::IsRunning()
{
	return m_IsRunning;
}

void ServiceV2::Sleep(double sleep_time_in_sec, void (*error_check)())
{
...
std::shared_ptr<Property> ServiceV2::GetProperty(const std::string &property_name) const
{
...
std::shared_ptr<Command> ServiceV2::GetCommand(const std::string &command_name) const
{
...
std::shared_ptr<DataStream> ServiceV2::GetDataStream(const std::string &stream_name) const
{
...
json ServiceV2::GetConfig() const
{
	return m_Config;
}

const std::string &ServiceV2::GetId() const
{
	return m_ServiceId;
}

void ServiceV2::MakeProperty(std::string property_name, Property::Getter getter, Property::Setter setter, DataType dtype)
{
...
void ServiceV2::MakeCommand(std::string command_name, Command::CommandFunction func)
{
...
std::shared_ptr<DataStream> ServiceV2::MakeDataStream(std::string stream_name, DataType type, std::vector<size_t> dimensions, size_t num_frames_in_buffer)
{
...
std::shared_ptr<DataStream> ServiceV2::ReuseDataStream(std::string stream_name, std::string stream_id)
{
...
std::shared_ptr<TestbedProxy> ServiceV2::GetTestbed()
{
	return m_Testbed;
}

string ServiceV2::HandleGetInfo(const string &data)
{
...
string ServiceV2::HandleGetProperty(const string &data)
{
...
string ServiceV2::HandleSetProperty(const string &data)
{
...
string ServiceV2::HandleExecuteCommand(const string &data)
{
...
string ServiceV2::HandleShutDown(const string &data)
{
	ShutDown();
...
void ServiceV2::UpdateState(ServiceState state)
{
