                                                ...
                                                Uuid trace_id, uint8_t memory_block_id)
{
	...
	else
	{
		...
		// Create a MessageHeader on the stack for the temporary buffer
		// We'll use a simplified approach - store the header info separately
		// and create the Message object
		
		// For now, create a dummy header - we'll serialize properly in PublishMessage
		MessageHeader* header = new MessageHeader();
		std::memset(header, 0, sizeof(MessageHeader));
		
		// Copy topic
		std::string topic_copy(topic);
		std::strncpy(header->topic, topic_copy.c_str(), TOPIC_MAX_KEY_SIZE - 1);
		...
		header->payload_info.memory_block_id = 0;  // Not used for temp buffers
		header->payload_info.offset_in_buffer = 0;
		
		// Set timestamp
		header->producer_timestamp = 0;  // Will be set on publish
		
		// Note: This is a simplified implementation. In practice, we'd need to manage
		// the header lifetime properly. For now, we accept that this leaks memory
		// and needs a proper implementation.
		
		return Message(header, buffer, 0, false);
	}
}
                                                               ...
                                                               std::vector<uint8_t>& buffer)
{
	...
	// Create message
	return Message(header, buffer.data(), header->partial_frame_id, true);
}
