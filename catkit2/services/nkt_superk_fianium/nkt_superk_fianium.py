...
class NktSuperk(Service):
    ...
    def open(self):
        ...
        funcs = {
            ...
            #'fianium_status': self.update_func(self.update_fianium_status)
        }
    ...
    def update_fianium_status(self):
    ...
    #get_base_temperature = read_register(registerReadS16, Fianium.REG_BASE_TEMPERATURE, ratio=0.1)
    #get_supply_voltage = read_register(registerReadU16, Fianium.REG_SUPPLY_VOLTAGE, ratio=0.001)
    #get_external_control_input = read_register(registerReadU16, Fianium.REG_EXTERNAL_CONTROL_INPUT, ratio=0.001)

    #get_power_setpoint = read_register(registerReadU16, Fianium.REG_OUTPUT_POWER_SETPOINT, ratio=0.1)
    #set_power_setpoint = write_register(registerWriteU16, Fianium.REG_OUTPUT_POWER_SETPOINT, ratio=0.1)

    #get_current_setpoint = read_register(registerReadU16, Fianium.REG_CURRENT_SETPOINT, ratio=0.1)
    #set_current_setpoint = write_register(registerWriteU16, Fianium.REG_CURRENT_SETPOINT, ratio=0.1)
