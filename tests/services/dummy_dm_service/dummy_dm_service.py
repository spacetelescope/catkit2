...
class DummyDmService(Service):
    def __init__(self):
        ...
        self.dm_shape = self.config['dm_shape']  # TODO

    def open(self):
        # Make channels streamable
        for channel in self.channel_names:
            setattr(self, channel, self.make_data_stream(channel, 'float64', [self.dm_shape], 20))  # TODO
            getattr(self, channel).submit_data(np.zeros(self.dm_shape,))  # TODO
