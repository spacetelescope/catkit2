...
Catkit2 uses a service-oriented architecture to operate the testbed. A Service can operate a hardware device, or perform more complex operations with other services such as control loops or safety mechanisms. Each service runs on a separate process to promote concurrent high-speed operations, and exposes an API to communicate with the outside world. This API is used for setting and getting parameters or execute methods, i.e. the exposure time of a camera or starting its acquisition loop, or for high-speed low-latency data sharing, for example camera images or deformable mirror commands. Services are managed by a Testbed object. The Testbed can start and stop services, manages configuration files and provides service discovery to allow scripts and other services to find services.
...
This object is a rolling buffer of fixed-size arrays. These arrays (or "DataFrame"s as they are called in catkit2) can be submitted to the data stream, at which point they are given a unique ID and appended to the end of the rolling buffer. DataStreams are located in shared memory, and are designed to be extremely fast, achieving single-digit microsecond latencies. Data frames can be accessed directly as the underlying memory is shared between processes. No information is exchanged with the Testbed server during this. DataStreams provide the backbone of high-speed communication between services and service proxies.

Data streams provide simultaneous access to data published by services. This allows for, for example, a camera to publish images on a data stream, which is then simultaneously accessed by A) the main adaptive optics process, B) a process that performs some long-term post-processing on the data, and C) a process that logs the taken images to disk and the graphical user interface. Note that all processes are running concurrently with little communication overhead.

DataStreams allow for simultaneous reading and writing, by any number of readers or writers. This allows for, for example, the adaptive optics process to write its deformable mirror (DM) commands to a data stream, which is the accessed by both the DM service to apply the shape on the DM, and a graphical user interface to provide feedback to the user.
