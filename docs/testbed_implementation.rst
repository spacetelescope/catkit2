...
simulates a simple pupil mask and a camera, and its parameters are read from ``config/model.yml``. By talking to the
simulated camera, can access the simulated camera images generated by the optical model.
