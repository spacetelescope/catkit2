...
**DOCUMENTED distinction:** an optical model and a simulator are different concerns. `OpticalModel` (and its concrete subclasses, e.g. `SimpleOpticalModel`) is a digital model of an optical train — a sequence of planes with propagation functions — that only knows how to propagate electrical fields between planes given some plane parameters; it has no knowledge of catkit2 services, the testbed or scheduling. `Simulator` is a `catkit2` service: it orchestrates simulated services on behalf of the testbed server (scheduling simulated time, receiving DM/stage/filter/power commands, driving camera acquisition) and holds an `OpticalModel` instance to which it forwards those calls instead of actuating hardware. For example, a simulated camera calls into the simulator, which reads the relevant field from its `OpticalModel` (via `get_camera_power`) rather than the optical model calling the camera directly. Keep plane physics/propagation logic in the optical model and service-orchestration/scheduling logic in the simulator; avoid collapsing the two into one class.
